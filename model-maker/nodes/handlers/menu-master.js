...
function update_menu_slider(self, data_name, display_name, data_affix="", limit_inputs=false){
    { // ADDING INPUT FIELD EVENTS
        // add input field listender
        let e = document.getElementById(`${self.uuid}${data_name}`) //check is the dataname that is being updated even exist
        if(e === null) return
        
        // if the slider indeed exists, then set its current value
        document.getElementById(e.id + "-slider").value = JSON.parse(
            localStorage.getItem(
                JSON.parse(localStorage.getItem(self.uuid)).data
            )
        )[data_name]
        ...
        e.addEventListener("focusout", (evt) => {
            if(document.getElementById(e.id + "-slider")){
                ...
                update_node_data(self, e.id.substring(0,e.id.length-data_name.length), data_name, e.value, e.value, name=display_name, affix=data_affix, "", true)
                sync_data_all()
            }
        })
    } { // ADDING SLIDER EVENTS
        let e = document.getElementById(`${self.uuid}${data_name}-slider`)
        e.onmousedown = (evt) => {
            e.onmousemove = (evt2) => {
                ...
                update_node_data(self, e.id.substring(0,e.id.length-data_name.length-7), data_name, e.value, e.value, name=display_name, affix=data_affix, "", true)
                sync_data_all()
            }
        }
        e.onmouseup = (evt) => {
...
function update_menu_dropdown(self, data_name, display_name, name_standard){
    let e = document.getElementById(`${self.uuid}${data_name}`) // check that is the dataname exists
    if(e === null) return
    ...
    e.onchange = (evt) => { // add the event listener
        update_node_data(self, e.id.substring(0,e.id.length-data_name.length), data_name, e.value, name_standard[e.value],
            name=display_name, affix="", "", true)
    }
}

function update_menu_toggle(self, data_name, display_name, toggle_functionality){
    let e = document.getElementById(`${self.uuid}${data_name}`) // check that is the dataname exists
    if(e === null) return
    ...
    e.onchange = (evt) => { // add the event listener
        update_node_data(self, e.id.substring(0,e.id.length-data_name.length), data_name, e.checked, e.checked?"Yes":"No",
